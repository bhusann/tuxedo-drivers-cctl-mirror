...
static int force_backlight_type = -1;
module_param(force_backlight_type, int, 0444);
MODULE_PARM_DESC(force_backlight_type, "Force keyboard backlight type (1=fixed, 2=3-zone RGB, 6=1-zone RGB, 243=per-key RGB)");
...
int clevo_leds_init(struct platform_device *dev)
{
	...
	if (force_backlight_type >= 0) {
		clevo_kb_backlight_type = force_backlight_type;
		pr_info("Forcing keyboard backlight type to: 0x%02x\n", clevo_kb_backlight_type);
	} else {
		for (i = 0; i < 3; ++i) {
